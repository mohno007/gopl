...
func Example_struct_map() {
	Display("m", map[struct {
		x int
		y int
	}]int{
		{0, 0}: 0,
	...
	/*
		//!+output
		// Display m (map[struct { x int; y int }]int):
		// x[struct { x int; y int }{ x: 0; y: 0; }] = 0
		// x[struct { x int; y int }{ x: 0; y: 1; }] = 1
		// x[struct { x int; y int }{ x: 0; y: 1; }] = 1
		// x[struct { x int; y int }{ x: 1; y: 1; }] = 1
	*/
}
